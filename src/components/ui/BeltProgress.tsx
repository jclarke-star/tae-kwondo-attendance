...
export function BeltProgress({ currentBelt, totalSessions }: BeltProgressProps) {
  ...
  const nextBelt = BELT_ORDER[currentIndex + 1] || "Master Level";
  // Logic: Every 10 sessions is a "milestone" towards the next belt for this demo
  const progressToNext = (totalSessions % 10) * 10; 
  ...
  return (
    <div className="space-y-4 w-full">
      <div className="flex justify-between items-end px-1">
        <div>
          <p className="text-[10px] font-black uppercase text-muted-foreground tracking-widest">Current Rank</p>
      ...
      <div className="relative h-10 w-full bg-slate-200 rounded-2xl border-4 border-black overflow-hidden shadow-playful-sm">
        ...
        <motion.div 
          ...
          className="absolute inset-y-0 left-0 bg-kidGreen"
        />
        {/* Belt Visual */}
        <div className="absolute inset-0 flex items-center justify-around px-4 pointer-events-none">
          ...
          <div className={cn("w-12 h-4 border-2 border-black rounded-sm opacity-40", getBeltColor(nextBelt))} />
        </div>
      </div>
      <div className="flex items-center justify-between bg-white border-2 border-black rounded-xl p-3 shadow-playful-sm">
        <div className="flex items-center gap-3">
          ...
          <div>
            <p className="text-xs font-black">{10 - (totalSessions % 10)} MORE CLASSES</p>
        ...
        <div className="flex -space-x-2">
          {[1, 2, 3].map((i) => (
            <div key={i} className={cn(
              ...
              i <= (totalSessions % 10) / 3 ? "bg-kidYellow" : "bg-white"
            )}>
              <Star className={cn("w-3 h-3", i <= (totalSessions % 10) / 3 ? "fill-black" : "text-black/20")} />
            </div>
          ))}
        </div>
      </div>
